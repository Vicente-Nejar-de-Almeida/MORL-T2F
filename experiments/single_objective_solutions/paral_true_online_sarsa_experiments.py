...
def run_experiment(params):
    alpha, lamb, fourier_order, df_all_feats, n_features, model, normalized_scorers, y_true, nameDataset, episodes_total = params
    ...
    for run in range(runs):
        ...
        file = open(f"results/true_online_sarsa_final_episodes_results_{nameDataset}.csv", mode='a',
                    newline='')
...
if __name__ == '__main__':
    ...
    for nameDataset in dataset_names:
        ...
        writer = csv.writer(file)
        writer.writerow(
            ['alpha', 'lambda', 'fourier_order', 'run', 'episode', 'rewards', 'features', 'y_pred', 'scores', 'AMI',
             'time',
             'silhouette', 'calinski_harabasz', 'davies_bouldin', 'dunn_index', 'silhouette_norm',
        ...
        for alpha in [0.001, 0.00001, 0.000001]:
            for lamb in [0.1, 0.45, 0.9]:
                for fourier_order in [2, 4, 7]:
                    params_list.append((alpha, lamb, fourier_order, df_all_feats, n_features, model, normalized_scorers,
